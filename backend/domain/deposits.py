...
class MonthlyDeposit:
    ...
    def __init__(self, start_date: date, periods: int, amount: int, monthly_rate: float):
    ...
    @classmethod
    def from_annual_rate(cls, start_date: date, periods: int, amount: int, annual_rate: float):
        ...
        if annual_rate < cls.MIN_ANNUAL_RATE or annual_rate > cls.MAX_ANNUAL_RATE:
            raise ValueError(f"Annual rate should be between [{cls.MIN_ANNUAL_RATE}, {cls.MAX_ANNUAL_RATE}]")

        monthly_rate = annual_rate / 12 / 100

        return cls(start_date=start_date, periods=periods, amount=amount, monthly_rate=monthly_rate)
    ...
    def start_date(self, start_date):
        if not isinstance(start_date, date):
            raise ValueError(f"Start_date should be 'date' type. {type(start_date)} type given")
    ...
    def periods(self, periods):
        if periods < self.MIN_PERIODS or periods > self.MAX_PERIODS:
            raise ValueError(f"Number of periods should be between [{self.MIN_PERIODS}, {self.MAX_PERIODS}]")
    ...
    def start_amount(self, start_amount):
        if start_amount < self.MIN_AMOUNT or start_amount > self.MAX_AMOUNT:
            raise ValueError(f"Start amount should be between [{self.MIN_AMOUNT}, {self.MAX_AMOUNT}]")
    ...
    def monthly_rate(self, monthly_rate):
        if (monthly_rate < self.MIN_ANNUAL_RATE / 12 / 100 or
                monthly_rate > self.MAX_ANNUAL_RATE / 12 / 100):
            raise ValueError(f"Annual rate should be between [{self.MIN_ANNUAL_RATE}, "
    ...
    def _calculate_deposit_periods_amount(self) -> List[DepositPeriodAmount]:
        ...
        for i in range(1, self.periods + 1):
            ...
            periods_amount.append(DepositPeriodAmount(date=next_date, amount=round(next_amount, 2)))
